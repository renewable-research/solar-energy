...
class ConvNeXtCustom(nn.Module):
    def __init__(self, num_classes, pretrained=True):
        super().__init__()
        base = create_model('convnext_base', pretrained=pretrained)
        self.features = nn.Sequential(*list(base.children())[:-1])
        num_ftrs = base.num_features
...
# Instantiate model
model = ConvNeXtCustom(num_classes=num_classes, pretrained=True).to(device)
...
for epoch in range(epochs):
    ...
    if val_acc > best_acc:
        best_acc = val_acc
        best_epoch = epoch + 1
        torch.save(model.state_dict(), "checkpoints/convnext_base_state_dict.ckpt")
...
plt.xlabel("Epoch")
plt.ylabel("Accuracy")
plt.title("ConvNeXt Training Accuracy")
plt.legend()
plt.grid(True)
plt.show()
...
model.load_state_dict(torch.load("checkpoints/convnext_base_state_dict.ckpt"))
model.eval()
