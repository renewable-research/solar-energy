...
# ================== Model ==================
# Use SqueezeNet v1.1 (lighter & faster than v1.0)
model = models.squeezenet1_1(pretrained=True)

num_ftrs = 512 
model.classifier = nn.Sequential(
    nn.AdaptiveAvgPool2d((1, 1)),  # ensure output is (batch, 512, 1, 1)
...
# Add softmax-like output (global average pooling is already included in SqueezeNet)
...
for epoch in range(epochs):
    ...
    if val_acc > best_acc:
        best_acc = val_acc
        best_epoch = epoch + 1
        torch.save(model.state_dict(), "checkpoints/squeezenet_state_dict.ckpt")
...
model.load_state_dict(torch.load("checkpoints/squeezenet_state_dict.ckpt"))
model.eval()
